...
function MapTileLayer({
    ...
    }, [context, name, url, attribution, isMapReady])

    if (!isMapReady || (context && context.selectedTileLayer !== name)) {
        return null
    }
...
function MapLocateControl({
    ...
    Pick<LocateOptions, "watch" | "timeout" | "maximumAge" | "enableHighAccuracy"> & {
    ...
    function startLocating() {
        ...
        map.locate({ 
            setView: true, 
            maxZoom: Math.min(16, map.getMaxZoom() || 18), 
            watch,
            timeout,
            maximumAge,
            enableHighAccuracy
...
interface MapDrawContextType {
    ...
    editControlRef: React.MutableRefObject<EditToolbar.Edit | null>
    deleteControlRef: React.MutableRefObject<EditToolbar.Delete | null>
}
